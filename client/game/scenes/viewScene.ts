...
class LevelScene extends Phaser.Scene {
    ...
    private updatePlayer(player:Player, direction: PlayerDirection | null, callback:CallableFunction ) {
        if (!direction) {
            //callback();
            return;
        }
        this.troToMovePlayer(player ,direction, callback);
    }

    private troToMovePlayer(player:Player, direction: PlayerDirection, callback:CallableFunction) {
    ...
    private createPlayers() {
        const playerSpawns = this.getSpawn();
    ...
    private getSpawn():Array<Phaser.Tilemaps.Tile> {
